...
The firmware runs on an [ESP32-WROOM-32 development board](https://www.amazon.co.uk/dp/B0DGLCWR76) connected to the Home Assistant host over USB. The one I used is sold as a diymore USB-C ESP32 development board with a CH340C USB-serial chip; diymore has a related [ESP32 development board product page](https://www.diymore.cc/products/diymore-esp32-development-board-wireless-wifi-bluetooth-dual-cores-ultra-low-power-consumption-cp2102-esp8266-esp-32-2-4ghz-module-integrated-with-antenna-rf-amp-filter-ap-sta-arduino-ide), and Espressif’s own [ESP32-DevKitC page](https://www.espressif.com/en/products/devkits/esp32-devkitc) is the useful manufacturer reference for this class of board. The ESP32 needs Bluetooth Classic support. That detail matters because not every ESP32-family board is suitable. Some newer or smaller variants focus on Wi-Fi and BLE, but the Wii Remote uses [Bluetooth Classic HID](https://www.bluetooth.com/specifications/specs/human-interface-device-profile-1-1-1/).
...
That gives the firmware one clean responsibility.
...
This is the difference between "I can read a button" and "I trust this enough to let it touch the house". Te same philosophy appears elsewhere.
...
That works, but a good Home Assistant integration should also feel native once it is installed. So the add-on supports MQTT Discovery. When discovery is enabled, it publishes retained discovery topics so Home Assistant creates entities automatically.
...
The blueprint exposes actions for a small set of common buttons such as `A`, `HOME` and `PLUS`.  It is enough to make common button mappings easy while keeping the lower-level MQTT topics available for anything more specific. The pattern I like looks like this:
...
The exact mapping is personal. The important part is that Home Assistant sees the Wii Remote as a stream of deterministic events. Once it is an event stream, it can do anything Home Assistant can do. The same topic structure also makes scaling straightforward. The current ESP32 Wiimote stack is effectively one controller per ESP32 radio. That sounds like a limitation, but it is a manageable one.
...
That keeps automations readable. A Wii Remote in the living room and a Wii Remote in the office do not need to share state or identity. Could the firmware eventually support multiple controllers on one ESP32? Unfortunately no. This limitation is due to the way Classic HID works (one radio one device). That is not true for more modern Bluetooth protocols like BLE. In any case, for this project, the cleaner path is one cheap ESP32 per Wii Remote. It is not elegant in the abstract. It is robust in the real world. I will take robust.
...
On Home Assistant OS, the ESP32 will normally appear as something like this.
...
The third common issue is MQTT configuration. The add-on defaults are meant to work with the official [Mosquitto broker add-on](https://github.com/home-assistant/addons/tree/master/mosquitto). But if you run MQTT elsewhere, the host, credentials, transport, and TLS settings need to match that broker. None of this is exotic. That is good. The failure modes are boring enough to diagnose.
...
That separation keeps the ESP32 firmware small, keeps Bluetooth away from Home Assistant, and gives the automation layer a simple MQTT contract. The bridge currently focuses on button events, connection state, heartbeat, and battery. That is enough to make it useful, but the Wii Remote has more hardware available. The obvious future work is still waiting.
...
The follow-up project is available here: [https://github.com/andremmfaria/ha-wiimote-bridge](https://github.com/andremmfaria/ha-wiimote-bridge)

The improved ESP32 Wiimote library is here: [https://github.com/andremmfaria/ESP32Wiimote](https://github.com/andremmfaria/ESP32Wiimote)
