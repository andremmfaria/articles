...
OpenClaw needs the same property through different machinery. The guard set is ported into [`openclaw/plugins/agent-config-guards/`](https://github.com/andremmfaria/agent-config/tree/main/openclaw/plugins/agent-config-guards), and [`openclaw/exec-approvals.json`](https://github.com/andremmfaria/agent-config/blob/main/openclaw/exec-approvals.json) keeps the coding-capable agents on `security` set to `full` and `ask` set to `off`, so reads and ordinary work never prompt, while research and planning roles use `security` set to `deny` and lose the execution tools entirely. The destructive gating itself lives in the plugin, so both runtimes ask on the same short list (`rm -r`, `git reset --hard`, `git clean`, force-push, and friends) and deny the same catastrophic and exfiltration patterns. I tried a strict per-agent allowlist first. It prompted on every `gh` call and every multi-line script, which is how you train yourself to click approve without reading. A gate that fires constantly is not a gate. Sandboxing is the same idea taken further. No gate to persuade because the capability is not there.
